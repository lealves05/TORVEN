...
| **Compras e materiais** | Sugestão de compra (mínimo/máximo, pedidos a caminho e demanda das OS); cotação com vários fornecedores e mapa de preços; pedido de compra por fornecedor; recebimento conferido (quantidade, custo, lote/corrida e certificado) com divergências registradas, custo médio e contas a pagar; cancelamento da entrada devolve o pedido; separação física do material para a OS |
| **Contas e conciliação** | Contas (caixa, bancos, cartão) com saldo; conta padrão por forma de pagamento; transferências fora do resultado; importação de extrato OFX/CSV sem duplicar; sugestão e conciliação manual ou automática segura; lançamento direto do extrato; conciliado não pode ser apagado |
| **Fluxo e resultado** | Fluxo de caixa projetado por semana (30 a 180 dias) com vencidos à parte; DRE gerencial mensal (regime de caixa, CMV pelas OS entregues) |
| **Relacionamento** | Retornos gerados automaticamente (pós-venda, orçamento sem resposta, garantia vencendo, manutenção, cobrança), registro do contato com nota 0–10 e NPS; nada é enviado sem ação do usuário |
| **Gestão e fiscal** | Indicadores: funil comercial, prazos médios, entregas no prazo, margem real por OS (materiais + horas apontadas + terceiros + comissões), retrabalho e produtividade por técnico; painel de OS entregues sem nota |
| **Dados** | Exportação CSV por assunto e cópia completa em JSON (sem senhas, tokens ou certificados), registradas na auditoria |
...
- `006_fase3_materiais.sql` (Fase 3): cotações, pedidos de compra, recebimento com lote/certificado, separação para OS.
- `007_fase4_financeiro.sql` (Fase 4): contas financeiras (com gatilho que define a conta pela forma de pagamento), transferências, extratos e conciliação.
- `008_fase5_gestao.sql` (Fase 5): retornos de relacionamento.
...
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f backend/src/migrations/rollback/008_fase5_gestao.down.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f backend/src/migrations/rollback/007_fase4_financeiro.down.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f backend/src/migrations/rollback/006_fase3_materiais.down.sql
...
Todos os scripts foram testados em sequência (008 → 004) numa cópia do banco, e a reaplicação posterior funciona.
O 008 apaga os retornos; o 007 apaga contas, extratos e conciliações; o 006 apaga cotações, pedidos de compra e dados de separação; o 005 apaga agenda, apontamentos, inspeções e garantias. O 004 apaga solicitações, contatos, endereços, anexos, revisões/aprovações e auditoria, e reconverte perfis e estados novos para os antigos.
...
custo real, inspeção obrigatória/reprovação, entrega com recebedor, garantia com retrabalho e restrições do perfil técnico; Fase 3: sugestão,
cotação, pedido, recebimento parcial/total com divergências, cancelamento, separação; Fase 4: contas, transferência, OFX/CSV, conciliação e bloqueios,
fluxo projetado e DRE; Fase 5: retornos idempotentes, NPS, indicadores, painel fiscal, exportação sem segredos — sempre com isolamento entre empresas. Build do site: `cd frontend && npm run build`.
...
│   │                      qualidade, garantias, compras (procurement), finanças, relacionamento, exportação, materiais,
