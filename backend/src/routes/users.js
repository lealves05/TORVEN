...
import { parse, notFound, HttpError } from '../util.js';

const r = Router();
r.use(need('users'));

const cols = 'id, name, email, role, technician_id, active, created_at';
...
const base = {
  ...
  role: z.enum(['admin', 'attendant', 'technician']),
...
r.post('/', async (req, res) => {
  ...
  const u = await one(
    `insert into users (company_id, name, email, password_hash, role, technician_id)
     values ($1,$2,$3,$4,$5,$6) returning ${cols}`,
    [req.companyId, d.name, d.email, await bcrypt.hash(d.password, 10), d.role, d.technician_id || null]);
...
r.put('/:id', async (req, res) => {
  const d = parse(z.object({
    ...base,
    role: z.enum(['owner', 'admin', 'attendant', 'technician']),
  ...
  const u = await one(
    `update users set name=$1, email=$2, role=$3, technician_id=$4, active=$5 where id=$6 returning ${cols}`,
    [d.name, d.email, d.role, d.technician_id || null, d.active ?? cur.active, cur.id]);
